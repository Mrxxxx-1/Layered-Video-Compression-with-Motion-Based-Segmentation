...
@njit(parallel=True)
def compute_motion_vectors(curr_frame_r, prev_frame_r, search_range=4):
    block_size = 16
...
def main():
    ...
    for idx, frame in enumerate(padded_frames):
        if idx > 0:
            ...
            # Perform DCT and compression
            compressed_frame = perform_dct_optimized(frame, block_size=8, quant_fg=quant_fg, quant_bg=quant_bg, foreground=foreground)
            compressed_frames.append(compressed_frame)
